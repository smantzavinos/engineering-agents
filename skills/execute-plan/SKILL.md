...
{{delegate:planner}}
Read PLAN_DIR/plan.md and tasks.json. Produce the interface surface the tasks share: exact function/type signatures, file paths, and data shapes crossing task boundaries. Write it to PLAN_DIR/interfaces.md. Do not implement anything.
{{/delegate}}
...
{{delegate:planner skill=create-tasks}}
...
{{delegate:oracle}}
Review the complete diff BASE..HEAD against PLAN_DIR/plan.md. You have not seen this work before. Report anything unfinished, unsafe, or inconsistent with the plan's intent.
{{/delegate}}
