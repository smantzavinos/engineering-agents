...
- [ ] The PATH `pi` command is repo-owned, delegates to the injected upstream Pi binary/store path, Node runtime, and shared-checker helper via injected absolute paths (never recursive PATH lookup), checks only documented interactive launches, and still keeps `--help`/`list` output clean.
- [ ] The notifier reads only `PI_MANAGED_PACKAGE_STARTUP_STATUS_PATH`, ignores expired/malformed/unowned files, sets a footer/status summary while startup problems are present, and consumes a valid snapshot once.
...
| `bash tests/specs/pi-startup-wrapper-spec.sh` | touched-files | During TDD loops for T3 | Wrapper argv classification, absolute-path runtime/helper handoff, env handoff, snapshot lifecycle, and fail-open startup behavior are correct |
| `bash tests/specs/startup-warning-extension-spec.sh` | touched-files | During TDD loops for T4 | The notifier renders stale/unknown distinctly, keeps footer/status summary in sync, and ignores or consumes snapshots correctly |
