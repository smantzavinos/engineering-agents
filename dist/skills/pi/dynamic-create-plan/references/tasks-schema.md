...
Dynamic route segments such as `[id]` are not expressible. Use a whole-segment
...
the two tasks and label it as such in `plan.md`. The files are disjoint, and
the edge exists only to satisfy the checker.
...
scope of it, typically a describe title grepped by the task's `verify`
command. Pin the titles in `interfaces.md` and keep scopes mutually exclusive.
No test title inside one task's scope may contain another task's grep
substring, or one task's verify will run another's tests.
