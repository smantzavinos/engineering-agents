...
| **SUB-10** | Crew actions require ephemeral session registration | `join` and `autoRegisterPath` are exempt from the registration check; every other Crew action requires registered state (`crew/index.ts:55-72`, `crew/handlers.ts:49-54`). | Join Messenger before the profile preflight in every Pi session. Registration is session-local, not persisted project state. |
| **SUB-11** | The bundled Crew worker conflicts with lead-only commits | The bundled worker directs every task through `git add -A`, `git commit`, and `task.done` commit evidence (`crew/agents/crew-worker.md Phase 5`). Discovery loads bundled agents first, then replaces same-name entries from `.pi/messenger/crew/agents/` (`crew/utils/discover.ts:116-127`). | The lead-only commit design requires a project `crew-worker` override, verified before dispatch, that preserves Crew coordination but forbids worker Git mutation and commit evidence. |
| **SUB-12** | Crew worker `tools` frontmatter drops Messenger | `crew/agents.ts` filters agent-frontmatter tools to `BUILTIN_TOOLS` before passing `--tools`, which drops the custom name `pi_messenger`, while `--tools` applies to extension tools too (`crew/agents.ts:40-41,225-245`). | The project `crew-worker` override must have no `tools` frontmatter. Pi defaults plus the explicitly loaded Messenger extension then remain available, preserving the full worker protocol. |
