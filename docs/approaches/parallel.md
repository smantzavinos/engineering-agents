...
Installed `dynamic-*` skills still describe two runners (ready-set waves vs one-shot DAG).
Those are the old split. Fold them into this model; do not invent a third.
...
Rules for when this lands in `tasks.json`:

- Every task id appears in exactly one group.
- Groups run in listed order.
- Edges inside a group are scheduled as a DAG.
- A missing `fenceGroups` means one group containing every task.

Until the schema and executor change, write the groups in `plan.md` so the intent is
visible. Do not fake them with extra serialization edges.
